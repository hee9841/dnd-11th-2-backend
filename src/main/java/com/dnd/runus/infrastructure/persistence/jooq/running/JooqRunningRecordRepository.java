...
public class JooqRunningRecordRepository {
    ...
    public List<RunningRecordWeeklySummary> findWeeklyDistanceSummaryMeter(
        ...
        //
        //        CommonTableExpression<Record2<Date, Integer>> sumTable = name(
        //            "sum_table")
        //            .fields("date", "sum_distance")
        //            .as(
        //                select(
        //                    cast(RUNNING_RECORD.START_AT, SQLDataType.DATE),
        //                    sum(RUNNING_RECORD.DISTANCE_METER).cast(Integer.class)
        //                )
        //                    .from(RUNNING_RECORD)
        //                    .where(RUNNING_RECORD.MEMBER_ID.eq(memberId))
        //                    .and(RUNNING_RECORD.START_AT.ge(startWeekDate))
        ...
        //                sumTable.field("sum_distance", SQLDataType.INTEGER)
        //            ).from(dateRange)
        //            .leftJoin(sumTable)
        //            .on(dateRange.field("data", SQLDataType.DATE)
        //                .eq(sumTable.field("date", SQLDataType.DATE)))
        //            .fetch(new RunningWeeklyDistanceSummary());
        ...
        //                .on(cast(RUNNING_RECORD.START_AT, SQLDataType.DATE).eq(dateRange.field("date",
        // SQLDataType.DATE)))
        //                .and(RUNNING_RECORD.MEMBER_ID.eq(memberId))
        //                .groupBy(dateRange.field("date"))
        //                .orderBy(dateRange.field("date"))
        //                .fetch(new RunningWeeklyDistanceSummary(dates));
    }
